#!/usr/bin/env python3
"""
Script para verificar transações do UmbrellaPay no sistema (versão direta - sem app.py)
Acessa o banco de dados diretamente para evitar problemas de importação
"""
...
def get_db_connection():
    ...
    db_name = os.environ.get('DB_NAME', 'grimbots')
    db_user = os.environ.get('DB_USER', 'grimbots')
    db_password = os.environ.get('DB_PASSWORD', '123sefudeu')
...
def verificar_transacoes():
    ...
    for transacao in TRANSACOES_GATEWAY:
        ...
        cursor.execute("""
            SELECT 
                id, payment_id, gateway_transaction_id, gateway_transaction_hash,
                status, amount, customer_user_id, customer_name, customer_username,
                created_at, paid_at
            FROM payments
        ...
        if not payment:
            ...
            cursor.execute("""
                ...
                FROM payments
                WHERE gateway_type = 'umbrellapag'
                  AND gateway_transaction_hash = %s
                LIMIT 1
            """, (gateway_id,))
            payment = cursor.fetchone()
        ...
        if not payment:
            ...
            cursor.execute("""
                ...
                FROM payments
                WHERE gateway_type = 'umbrellapag'
                  AND customer_user_id LIKE %s
                  AND amount >= %s
                  AND amount <= %s
                LIMIT 1
            """, (f'%{gateway_cpf}%', gateway_valor - 0.01, gateway_valor + 0.01))
            payment = cursor.fetchone()
        ...
        if not payment:
            ...
            cursor.execute("""
                ...
                FROM payments
                WHERE gateway_type = 'umbrellapag'
                  AND customer_username LIKE %s
                  AND amount >= %s
                  AND amount <= %s
                LIMIT 1
            """, (f'%{telefone_limpo}%', gateway_valor - 0.01, gateway_valor + 0.01))
            payment = cursor.fetchone()
        
        if payment:
            # Verificar se o valor está correto
            valor_diferenca = abs(float(payment['amount']) - gateway_valor)
            ...
            if payment['status'] == 'paid':
                encontradas_pagas.append({
                    ...
                    'payment_valor': float(payment['amount']),
